...
export default function Home() {
  ...
  return (
    <main className="bg-[#050505] overflow-x-hidden text-[#FAF7F2]">
      ...
      <AnimatePresence>
        {mostrarWaFlotante && (
          <motion.a
            ...
            target="_blank"
            rel="noreferrer"
            className="fixed bottom-8 right-6 md:right-10 z-40 bg-[#25D366] text-white p-4 rounded-full shadow-[0_0_20px_rgba(37,211,102,0.4)] flex items-center justify-center cursor-pointer hover:bg-[#20bd5a] transition-colors"
          >
      ...
      {/* ════ MODAL ZOOM (CON Z-INDEX ALTO) ════ */}
      <AnimatePresence>
        {imagenZoom && (
          <motion.div onClick={() => setImagenZoom(null)} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] bg-black/90 backdrop-blur-sm flex items-center justify-center p-4 md:p-12 cursor-zoom-out">
            ...
            <button onClick={() => setImagenZoom(null)} className="absolute top-6 right-6 text-white/50 hover:text-white text-5xl font-light leading-none">×</button>
          </motion.div>
        )}
      </AnimatePresence>
              ...
              {productos.map((prod, i) => (
                <motion.div key={prod.id} {...reveal(0.1 * i)} className={`group bg-[#050505] border border-white/5 overflow-hidden flex flex-col md:flex-row rounded-2xl ${prod.offset ? 'md:translate-y-12' : ''}`}>
                  ...
                  <div className="w-full md:w-2/5 aspect-[7/10] overflow-hidden relative cursor-zoom-in" onClick={() => setImagenZoom(getImageUrl(prod.img))}>
                    ...
                    <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md px-4 py-1.5 rounded-full z-10 pointer-events-none">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
              {eventos.map((evento, i) => (
                <motion.div key={evento.id} {...reveal(0.1 * i)} onClick={() => setImagenZoom(getImageUrl(evento.img))} className="relative overflow-hidden aspect-[7/10] group cursor-zoom-in border border-white/5 rounded-2xl">
                  ...
                  <div className="absolute bottom-0 left-0 p-6 md:p-8 translate-y-4 group-hover:translate-y-0 transition-transform duration-500 w-full z-20 pointer-events-none">
